...
- 台账状态机：`持有中 → 展期中 → (到期离场|技术离场|止损出局)`（趋势·变体B），`持有中 → 止盈1减半 → (止盈2清仓|止损出局|技术离场|时间止损)`（短线）
- **右侧趋势·变体B「让利润奔跑」**（2026-09-14 落地）：-6% 硬止损（最优先）+ 收盘破 MA20 结构止损 + 破 MA10 移动止盈（无盈利门槛）；**去掉了固定双档止盈**（原 +8%减半/+10%清仓，回测 119 笔变体B 胜率31.9%×盈亏比4.01=127.9，优于基准 42.0%×1.97=82.7）
- **短线止损 -5.5% / 止盈1 +5% 减半后移动止损**（盈利>3%破MA10清仓），另加 3天浮亏时间止损（2026-09-09 回测：止损-4%被扫60%劣化，出场改造使短线转正）
- **技术面破坏自动剔除**：波段/展期票 收盘破 MA20 结构止损、破 MA10 移动止盈（变体B）；短线破 MA5 → 生命线失守

### 展期机制（右侧趋势）

- 满 28 天触发展期评估（小概率转中长期）：盈利>0、价>MA10>MA20、名额<2 才可展期
- **展期硬约束**：同时 ≤2 只（`TREND_EXTEND_MAX_COUNT`）、最长 60 天（`TREND_EXTEND_MAX_DAYS`）强制离场
- **展期后无固定止盈**：继续 MA10 移动止盈 / MA20 结构止损跟踪（变体B 让利润奔跑）

### 选股引擎

- 评分体系：价值30 + 技术30 + 资金25 + 题材15 = 100，≥70 推荐 / ≥80 强烈推荐
- **右侧追启动**：放量(量比≥1.5)站上MA20、近20日振幅<25%横盘紧凑、60日位置 30%~70%（2026-09-09：原"突破20日新高"与低位互斥致 0 票已删；同日加位置下限 0.30——深跌弱势趋势未确认，回测胜率 46.8→50.7%、均收+2.59→+3.39%）；温度≥55 才开趋势仓
- **短线冲高回落剔除**：当日最高相对现价回撤>3% 的冲高回落票不进短线候选（回测胜率 45.0→47.2%、盈亏比 1.59→1.69）
- **涨停剔除**：现价/涨停价 ≥0.95（近涨停）或 5 日内连板≥2 → 剔除（解决"好看不好买"）
- **行业去重**：同行业最多 1 只（`MAX_SAME_INDUSTRY`）
- **行业主线热度追踪（2026-09-10 新增）**：不设固定行业白名单，改按**板块热度持续性**判定主线——`data/industry_heat.csv`（同花顺 90 二级行业板块，backfill_industry_heat.py 增量维护）近5日板块涨幅进前15的天数：≥3日=🔥主线 / ≥1日=🌤升温 / 0=❄️冷门。复盘输出「持续主线行业」概览，观察/追踪表带行业热度列。行业名归一化：`_match_heat_board`（精确→子串→前缀→截尾）+ `_norm_ind`（去罗马后缀+同义词表）。日度刷新走 `_refresh_heat_today`（THS 板块指数K线，today 无数据自动落最近交易日，防新浪/东财名混写）
- **热度归因落地（2026-09-10）**：回测 16743 条低位信号×146 交易日，热度对**转化率几乎无增益**（冷门10.0% vs 主线11.1%），但**提升7日胜率**（46.2→48.0%；叠加前兆剔劣后 45.5→**53.3%**、7日均收 -0.06→+0.49%）；窗口敏感性 W5 显著、W10/W20 衰减 → **短期效应** → 落地为**排序加分不硬过滤**（`industry_heat_tool` 纯函数供 screener/backfill_watch/close_review 复用；低位池 🔥主线 +1 分排序，开关 `LOW_POS_HEAT_BONUS`/`LOW_POS_HEAT_MAIN_ONLY`；粗分类经 `GROSS_TO_BOARDS` 组映射）
- **代码白名单**：主板(60/00)+创业板(30)；排除科创板(68)/北交所(92/43/83)——无交易权限

### 低位启动观察池（low_pos_watch，2026-09 研究落地）

捕捉「蓄势贴前高、尚未启动」的低位股，供趋势池补充参考。进入观察池 ≠ 买入推荐，转化研究（`analyze_transform.py`）基线 10.5% → 剔劣三件套 16.1% → 组合 17.5%。

**剔劣三件套**（提高「入选→真正启动」转化率的硬过滤）：
- A 振幅：近20日振幅 < 15%（`LOW_POS_MAX_20D_AMP`）
- B 波动：近20日收益波动 < 4%（`LOW_POS_MAX_20D_STD`）
- C 位置：距60日高点 > -22%（`LOW_POS_MIN_DIST_60D_HIGH`）——贴前高蓄势，非深跌下行通道
- 位置/成交额：位置 20%~40%（`LOW_POS_BEST_POS`）、成交额 2~8 亿（`LOW_POS_BEST_AMOUNT`）为硬过滤（`LOW_POS_HARD_POS_AMOUNT=True`，2026-09-12 由排序加分升级为硬过滤）

**⚠️ 启动豁免博弈（必须记住，2026-09-11 加）**：
...
- 博弈本质：**蓄势期要求"没涨、贴前高"，一旦真放量启动反而要宽容放行**。收紧与豁免是一体两面，改蓄势过滤时务必同步保住这条豁免，否则又会把涨停启动票筛掉。
- 关联改动：涨跌幅过滤只保留下跌端 `>= -4`（上涨端不挡，否则涨停被当"过热"剔除）；涨停剔除只剔**一字板**（今开≥涨停价×0.995，无换手无法介入），换手板保留并标 `_breakout_tag`。

**连续天数**：用「首现日→今日」的交易日跨度（交易日历取上证指数 000001 K线日期），不是入选次数——否则未入选日断档会倒挂（4→3）。
...
| `TAKE_PROFIT_1/2` | +8%/+10% | 趋势双档止盈（减半/清仓） |
| `MOVE_STOP` | MA10 | 盈利>3%上移成本线后跌破MA10离场 |
| `SHORT_STOP_LOSS` | -5.5% | 短线止损（2026-09-09 回测校准：-4% 在1.9天持有期被扫60%劣化） |
